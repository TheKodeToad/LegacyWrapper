...
public class AppletFrame extends JFrame {
    ...
    public AppletFrame(AppletArguments args, Applet applet) {
        super(args.title);

        launcher = new Launcher(applet, null, args);
        ...
        try {
            ...
            Log.error("Failed to read window icon", exception);
        }

        addWindowListener(new ForceExitHandler());
    }

    public void start(AppletArguments args) {
        add(launcher);

        launcher.setPreferredSize(new Dimension(args.width, args.height));
